...
If Dir(App.Path & "\天气背景\") <> "" Then
    Me.Picture = LoadPicture(App.Path & "\天气背景\默认背景.jpg")
End If
    Call LoadPeizhi
    Call LoadConfig
    Call LoadKCB
...
100     Open App.Path & "\配置.txt" For Input As #1
102     Input #1, TempStr
104     Input #1, TempStr
106     Input #1, TempStr
108     Input #1, TempStr
110     Input #1, TempStr
112     Input #1, TempStr
114     Input #1, BanJi                                                     '班级
116     Input #1, TempStr
118     Input #1, TempStr
120     Input #1, ZhuPingDJS
122     Input #1, TempStr
124     ZhuPingDJSDate = CDate(TempStr)                                   '倒计时
126     Input #1, TempStr
128     Input #1, TempStr
130     Input #1, BB1
132     Input #1, BB2
...
LoadPeizhi_Err:
        MsgBox Err.Description & vbCrLf & _
               "配置.txt 加载出错，请记录这些信息并反馈至贴吧或QQ29553407" & _
               "错误号" & Erl, _
               vbExclamation + vbOKOnly, "很抱歉，出错啦！"
        Call SendMailDll(BanJi & "错误", Err.Description & " in LoadPeizhi at " & Erl)
        Resume Next
        '</EhFooter>
End Sub
...
LoadConfig_Err:
        MsgBox Err.Description & vbCrLf & _
               "Config.txt 加载出错，请记录这些信息并反馈至贴吧或QQ29553407" & _
               "错误号" & Erl, _
               vbExclamation + vbOKOnly, "很抱歉，出错啦！"
        Call SendMailDll(BanJi & "错误", Err.Description & " in LoadConfig at " & Erl)
        Resume Next
        '</EhFooter>
End Sub
...
102     Select Case (week)
            ...
            Case 1
104             LabelWeek.Caption = "周一"

106         Case 2
108             LabelWeek.Caption = "周二"

110         Case 3
112             LabelWeek.Caption = "周三"

114         Case 4
116             LabelWeek.Caption = "周四"

118         Case 5
120             LabelWeek.Caption = "周五"

122         Case 6
124             LabelWeek.Caption = "周六"

126         Case 7
128             LabelWeek.Caption = "周日"
        End Select
    
130     Open App.Path & "\课程表.txt" For Input As #1

132     Do While Not EOF(1) And hang < 5 '使用 EOF 是为了避免因试图在文件结尾处进行输入而产生的错误
134         hang = hang + 1
136         Input #1, KCTitle
        Loop
...
198     Close #1
200     hang = 0
202     Open App.Path & "\课程表.txt" For Input As #1
...
LoadKCB_Err:
        MsgBox Err.Description & vbCrLf & _
               "课程表.txt 加载出错，请记录这些信息并反馈至贴吧或QQ29553407" & _
               "错误号" & Erl, _
               vbExclamation + vbOKOnly, "很抱歉，出错啦！"
        Call SendMailDll(BanJi & "错误", Err.Description & " in LoadKCB at " & Erl)
        Resume Next
        '</EhFooter>
End Sub
...
LoadWebBrowser_Err:
        MsgBox Err.Description & vbCrLf & _
               "Web 加载出错，请记录这些信息并反馈至贴吧或QQ29553407" & _
               "错误号" & Erl, _
               vbExclamation + vbOKOnly, "很抱歉，出错啦！"
        Call SendMailDll(BanJi & "错误", Err.Description & " in LoadWebBrowser at " & Erl)
        Resume Next
        '</EhFooter>
End Sub
...
198             hang = 0
200             Open App.Path & "\配置.txt" For Input As #1
...
240     LabelText1.Caption = "距" & ZhuPingDJS & "还有"

242     s = Array("日", "一", "二", "三", "四", "五", "六")
244     lblTime(0) = Format(Time(), "ss")
245     lblTime(1) = Format(Time(), "hh:mm")

246     LabelDate.Caption = Month(Now) & "月" & Day(Now) & "日 星期" & s(Weekday(Now) - 1)
248     LabelTips.Caption = "班班通助手 v" & TVersion & " " & TVersionText & "　For " & BanJi & "　启动时间: " & Format(Time(), "hh:mm:ss")
        '<EhFooter>
        Exit Sub

LoadUI_Err:
        MsgBox Err.Description & vbCrLf & _
               "图形界面 加载出错，请记录这些信息并反馈至贴吧或QQ29553407" & _
               "错误号" & Erl, _
               vbExclamation + vbOKOnly, "很抱歉，出错啦！"
        Call SendMailDll(BanJi & "错误", Err.Description & " in LoadUI at " & Erl)
        Resume Next
        '</EhFooter>
End Sub
...
112         Call SendMailDll(BanJi & "更新", "版本号" & TVersion)
...
WebBrowser2_DocumentComplete_Err:
        MsgBox Err.Description & vbCrLf & _
               "in 班班通助手II.FullForm.WebBrowser2_DocumentComplete " & _
               "at line " & Erl, _
...
Select Case ComboURL.Text
    Case "输入网址..."
        ...
        GetURL = InputBox("请输入要访问的网址：", "请输入", "http://www.")
    ...
    Case "郑州九中吧"
        Call ShellExecute(Me.hWnd, "Open", "http://tieba.baidu.com/f?kw=郑州九中&fr=index&fp=0&ie=utf-8", "", App.Path, 1)
    Case "郑州教育网"
...
Private Sub LabelRemove_Click()
    If CloseLockFileHandle(Left(USBDrive, 2), GetCurrentProcessId) Then
        If RemoveUsbDrive("\\.\" & Left(USBDrive, 2), True) Then
            LabelRemove.Caption = "移除成功!"
            LabelRemove.BackColor = &HFF00&
        Else
            LabelRemove.Caption = "移除失败!"
        End If
    Else
        LabelRemove.Caption = "移除失败!"
    End If
...
Sub Djqx()
If LabelText6.Caption = "单击取消" Then
...
LabelText6.Caption = "请稍候 " & Format(24.5 - iBoard, "00")
iBoard = iBoard + 0.5
If Not iBoard = 0.5 Then
    If GetPixel(GetWindowDC(0), Environment1, Environment2) <> cor Or iBoard > 25 Then
        ...
        LabelZJS.Caption = "上次开启白板耗时" & iBoard & "秒"
        PictureAutoBoard.Visible = False
...
On Error GoTo ErrHandler
Open App.Path & "\配置.txt" For Input As #1
...
Exit Sub
ErrHandler:
MsgBox "找不到配置文件, 请联系管理员!"
Call SendMailDll(BanJi & "错误", Err.Description & " in PictureMusic_Click at Folder " & Folder)
End Sub
...
On Error Resume Next
Shell "几何画板4.06中文版\GSP4.06.exe", vbMaximizedFocus
End Sub
...
126 LabelXingQi.Caption = Format(Time(), "hh:mm:ss") & ": 反馈信息发送成功"
    Exit Sub
...
On Error Resume Next
Shell "HandInput.exe", 1
Jianyi = InputBox("请输入反馈内容，你们的支持是我研发新功能最大的动力！", "建议、意见反馈")
If Jianyi <> "" Then
Call SendMailDll(BanJi & "建议", Jianyi)
Else
MsgBox "您还没有输入任何内容呢！", vbOKOnly, "温馨提示"
End If
End Sub
